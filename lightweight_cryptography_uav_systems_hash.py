...
def benchmark_algorithm(name, path, hash_name, hash_func, results):
    print(f"\nTesting {name} with {hash_name}...\n")
...
def save_results_to_csv(results, filename):
    with open(filename, mode='w', newline='') as csvfile:
        fieldnames = ['Algorithm', 'Hash', 'Timestamp', 'CPU Usage (%)', 'RAM Usage (MB)', 'Voltage (V)', 'Current (mA)', 'Power (mW)']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for algo_hash, data in results.items():
            for i in range(len(data['timestamps'])):
                algo, hash_name = algo_hash.split('-')
                writer.writerow({
                    'Algorithm': algo,
                    'Hash': hash_name,
...
def main():
    ...
    result_dir = f"benchmark_results_{timestamp}"
    ...
    for algo, path in ALGORITHMS.items():
        for hash_name, hash_func_name in HASH_ALGORITHMS.items():
            hash_func = getattr(hashlib, hash_func_name)
            benchmark_algorithm(algo, path, hash_name, hash_func, results)
